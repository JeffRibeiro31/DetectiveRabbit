...
window.addEventListener ('load', () => {
    ...
    pig.style.left='260px';
    pig.style.top='100px';
    //-------------posição inicial do guarda2
    pig2.style.position = 'absolute';
    pig2.style.left='630px';
    pig2.style.top='280px';
    //-------------posição inicial do guarda2
    pig3.style.position = 'absolute';
    pig3.style.left='940px';
    pig3.style.top='280px';

    //-------------posição inicial do guarda2
    nextSign.style.position = 'absolute';
    nextSign.style.left='30px';
    nextSign.style.top='445px';
    ...
    for ( i=0; i < guarda.length; i++){
        for (j=0; j < rabbitactual.length; j++){
            if(rabbitactual[j].id === guarda[i].id){
               // alert("Pego!");
                if(!alert("Game Over! Você entrou no raio de visão do segurança... Tente novamente!")) document.location = "./home.html";
                break;
            }
        }
    }
    ...
    for ( i=0; i < guardapos.length; i++){
        for (j=0; j < rabbitactual.length; j++){
            if(rabbitactual[j].id === guardapos[i].id){
               // alert("Pego!");
                if(!alert("Game Over! Você passou muito perto do segurança... Tente novamente!")) document.location = "./home.html";
                break;
            }
        }
    }
...
function nextStage() {
   ...
   // console.log("Teste rabbit", rabbitactual);
    ...
    for ( i=0; i < rabbitactual.length; i++){
        if(rabbitactual[i].id === "13-2" || rabbitactual[i].id === "13-3" || rabbitactual[i].id === "14-2" || rabbitactual[i].id === "14-3" ){
